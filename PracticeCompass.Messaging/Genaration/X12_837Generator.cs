...
namespace PracticeCompass.Messaging.Genaration
{
    public class X12_837Generator
    {
        public void Generate837PMessage(List<ClaimMessageModel> ClaimMessageModel, string segmentSeparator, string FieldSeparator, string unknownplaceholder)
        {
            ...
            var GenerateISA = new GenerateISAsegment(ClaimMessageModel[0],FieldSeparator, unknownplaceholder);
            ...
            #endregion
            var fillingcodes = ClaimMessageModel.Select(x => x.FilingCode).Distinct().ToList();
            var loopindex = 0;
            for (var f= 0;f < fillingcodes.Count;f++)
            {
                Log.LogInfo("fillingcodes : " + fillingcodes.Count, "", "");
                loopindex++;
                ...
                #endregion
                var filteredbyfilling = ClaimMessageModel.Where(c => c.FilingCode == fillingcodes[f]).ToList();
                for (var l=0;l<filteredbyfilling.Count;l++)
                {
                    Log.LogInfo("filteredbyfilling : " + filteredbyfilling.Count,"","");
                    var claimnumbers = filteredbyfilling.Select(x => x.ClaimNumber).Distinct().ToList();
                    for (var c = 0; c < claimnumbers.Count; c++)
                    {
                        Log.LogInfo("claimnumbers : " + claimnumbers.Count, "", "");
                        loopindex++;
                        var rowsbyclaimnumber = ClaimMessageModel.Where(x => x.ClaimNumber == claimnumbers[c]).OrderBy(x => x.CoverageOrder).ToList();
                        //create 2000B HL segment 
                        var loop2000BHL = new Generateloop2000Bsegment(rowsbyclaimnumber[0], FieldSeparator, unknownplaceholder,parentlooindex,loopindex);
                        ...
                        {
                            Log.LogInfo("rowsbyclaimnumber : " + rowsbyclaimnumber.Count, "", "");
                            //create 2000B SBR 
                            var loop2000Bsbr = new Generateloop2000Bsegment(rowsbyclaimnumber[r], FieldSeparator, unknownplaceholder,0,0);
                            ...
                            //create 2010BB
                            #region Loop: 2010BA
                            var Loop2010BA = new Generateloop2010BAsegment(rowsbyclaimnumber[r], FieldSeparator, unknownplaceholder);
                            ...
                            #endregion
                            #region Loop: 2010BB
                            var Loop2010BB = new Generateloop2010BBsegment(rowsbyclaimnumber[r], FieldSeparator, unknownplaceholder);
                            ...
                            #endregion
                            if (rowsbyclaimnumber[r].CoverageOrder == 1)
                            {
                                //create 2300 
                                #region Loop: 2300
                                var loop2300 = new Generateloop2300segment(rowsbyclaimnumber[r], FieldSeparator, unknownplaceholder);
                                ...
                                #endregion
                                #region Loop: 2310A
                                var loop2310A = new Generateloop2310Asegment(rowsbyclaimnumber[r], FieldSeparator, unknownplaceholder);
                                var loop2310A_NM1 = loop2310A.GenerateLoop2310A_NM1_segment();
                                transactionEnvelope.Segments.Add(loop2310A_NM1);
                                #endregion
                                #region Loop: 2310B
                                var loop2310B = new Generateloop2310Bsegment(rowsbyclaimnumber[r], FieldSeparator, unknownplaceholder);
                        ...
                        {
                            Log.LogInfo("charges : " + charges.Count, "", "");
