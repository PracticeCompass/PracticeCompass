  ...
  (searchCriteria) =>
    async (dispatch, getState) => {
      try {
        ...
        if (searchCriteria.lookupFilter == "UserLookup") {
          const resp = await axios({
            method: "GET",
            url: `${config.baseUrl}/Lookup/UserLookupCodeGet?LookupType=${searchCriteria.LookupType}&IsActive=${searchCriteria.IsActive}&lookupCode=${searchCriteria.lookupCode}`
          });
  ...
  (data) =>
    async (dispatch, getState) => {
      try {
        ...
        await dispatch(setLookupCodes([]));
        if (data == null) return;
        if (data.lookupFilter == "UserLookup") {
          const resp = await axios({
            method: "GET",
            url: `${config.baseUrl}/Lookup/AddUserLookupCode?LookupType=${data.LookupType}&LookupCode=${data.LookupCode}&description=${data.description}&RecordStatus=${data.RecordStatus}&IsAdd=${data.IsAdd}&gridId=${data.gridId}`
          });
          return resp.data;
        } else {
          const resp = await axios({
    ...
    async (dispatch, getState) => {
      try {
        ...
        if (data.lookupFilter == "UserLookup") {
          const resp = await axios({
            method: "GET",
            url: `${config.baseUrl}/Lookup/AddUserLookupType?lookupType=${data.lookupType}&description=${data.description}&descriptionLabel=${data.descriptionLabel}&Class=${data.Class}&Length=${data.Length}`
          });
          return resp.data;
        } else {
          const resp = await axios({
  ...
  (search, lookupFilter) =>
    async (dispatch, getState) => {
      try {
        ...
        if (lookupFilter == "UserLookup") {
          const resp = await axios({
            method: "GET",
            url: `${config.baseUrl}/Lookup/userLookupTypeGet?search=${search}`,
          });
