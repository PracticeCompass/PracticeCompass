...
export const getclaims =
  ...
  async (dispatch, getState) => {
    try {
      ...
      const resp = await axios({
        method: "GET",
        url: `${config.baseUrl}/ClaimList/ClaimGridGet?PatientID=${
          searchCriteria.PatientID
        }&PracticeID=${searchCriteria.PracticeID}&DOSType=${
          searchCriteria.DOSType
        }&PhysicianID=${searchCriteria.PhysicianID}&GuarantorID=${
...
export const getPhysicianList =
  (name, refreshData, skip) => async (dispatch, getState) => {
    try {
      ...
      const resp = await axios({
        method: "GET",
        url: `${config.baseUrl}/ClaimList/PhysicianGet?sortname=${
          name === null ? "" : name
        }&skip=${skip}`,
      });
